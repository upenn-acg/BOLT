...
#include "bolt/Passes/InjectPrefetchOuterLoopPass.h"
...
bool InjectPrefetchOuterLoopPass::runOnFunction(BinaryFunction &BF) {
  ...
  llvm::outs()<<"[InjectPrefetchOuterLoopPass] The starting address of "<<demangledFuncName<<" is: 0x"
              <<utohexstr(startingAddr)<<"\n";
  llvm::outs()<<"[InjectPrefetchOuterLoopPass] The top llc miss addr is: 0x"
  ...
  std::unordered_map<BinaryBasicBlock*, std::vector<uint64_t> > TopLLCMissAddrGroups;
    ...
    for (auto It = BB.begin(); It != BB.end(); It++){
      ...
      if (BC.MIB->hasAnnotation(Instr, "AbsoluteAddr")){
        ...
        if (TopLLCMissAddrs_set.find(AbsoluteAddr) != TopLLCMissAddrs_set.end()){
          llvm::outs()<<"[InjectPrefetchOuterLoopPass] find instruction that causes the TOP LLC miss\n";
          if (BC.MIB->isLoad(Instr)){
            llvm::outs()<<"[InjectPrefetchOuterLoopPass] TOP LLC miss instruction is a load\n";           
          }
          else if (BC.MIB->isStore(Instr)){
            llvm::outs()<<"[InjectPrefetchOuterLoopPass] TOP LLC miss instruction is a store\n";
          }
          else {
            llvm::outs()<<"[InjectPrefetchOuterLoopPass] This pass only work for injecting prefetch for load/store instruction\n"; 
            return false;
          }
          if (TopLLCMissAddrGroups.find(&(*BBI)) != TopLLCMissAddrGroups.end()){
            TopLLCMissAddrGroups[&(*BBI)].push_back(AbsoluteAddr);
          }
          else {
            std::vector<uint64_t> instrAddrs{AbsoluteAddr};
            TopLLCMissAddrGroups.insert(std::make_pair(&(*BBI), instrAddrs));
          } 
        }
      }
    }
  ...
  for (auto it = TopLLCMissAddrGroups.begin(); it != TopLLCMissAddrGroups.end(); it++){
    ...
    for (auto I = TopLLCMissBB->begin(); I != TopLLCMissBB->end(); I++){
      if (BC.MIB->hasAnnotation(*I, "AbsoluteAddr")){
        ...
        if (TopLLCMissAddrs_set.find(AbsoluteAddr) != TopLLCMissAddrs_set.end()){
          TopLLCMissInstrsInThisBB.push_back(&(*I));
        }
      }
    }
  ...
  while (!LoopContainsTopLLCMissInstrs.empty()){
    ...
    SmallVector<BinaryBasicBlock*, 2> Latches;
    ...
    SmallVector<BinaryBasicBlock*, 2> PredsOfHeaderBB = HeaderBB->getPredecessors();
    ...
    BinaryBasicBlock* PopRegBB = createPopRegBB (BF, HeaderBB, BoundsCheckBB,
                                                 PrefetchBB, freeReg);
    ...
    PrefetchBB->addSuccessor(PopRegBB);
    BoundsCheckBB->addSuccessor(PopRegBB);

    // add Predecessors to HeaderBB
    HeaderBB->addPredecessor(PopRegBB); 

    // create Branch Instruction at the end of 
    // BoundsCheckBB
    MCInst BoundsCheckBranch;
    BC.MIB->createJZ(BoundsCheckBranch, PrefetchBB->getLabel()  , BC.Ctx.get());
    BC.MIB->reverseBranchCondition(BoundsCheckBranch, PrefetchBB->getLabel(),
                           BC.Ctx.get());
    BoundsCheckBB->addInstruction(BoundsCheckBranch);

    PrefetchBB->addBranchInstruction(PopRegBB);  
    // change HeaderBB's original predecessors' tail branch targets
    // to be BoundsCheckBB
    std::unordered_set<BinaryBasicBlock*> LatchSet;
    ...
    for (unsigned i=0; i<PredsOfHeaderBB.size(); i++){
      ...
      if (LatchSet.find(PredsOfHeaderBB[i])!=LatchSet.end()) continue;
...
BinaryLoop* InjectPrefetchOuterLoopPass::getWorkingLoop( BinaryFunction& BF,
  ...
  if (LoopDepth == 0) {
    llvm::outs()<<"[InjectPrefetchOuterLoopPass] The loop that contains top LLC miss BB doesn't exist\n";
    return NULL;
  }
...
void InjectPrefetchOuterLoopPass::getLoopGuardInstrs( BinaryFunction& BF,
...
std::pair<MCInst*, BinaryBasicBlock*> InjectPrefetchOuterLoopPass::findDemandLoad(BinaryFunction& BF,
...
BinaryBasicBlock* InjectPrefetchOuterLoopPass::createBoundsCheckBB(BinaryFunction& BF,
  ...
  for (auto predecessor: PredsOfTargetBB){
    if (LatchSet.find(predecessor)==LatchSet.end())
      predecessor->removeSuccessor(TargetBB);
  }
  // change the pred BB of TargetBB
  TargetBB->removeAllPredecessors();
  for (auto latch: Latches){
   TargetBB->addPredecessor(latch);
  }
...
*/
  ...
  for (unsigned i=0; i<PredsOfTargetBB.size(); i++){
    if (LatchSet.find(PredsOfTargetBB[i])==LatchSet.end())
...
BinaryBasicBlock* InjectPrefetchOuterLoopPass::createPrefetchBB(BinaryFunction& BF,
                                      BinaryBasicBlock* TargetBB,
  ...
  if (TopLLCMissInstr->getOperand(4).isExpr()){
    ...
    for (unsigned i=0; i<TopLLCMissInstrs.size(); i++){
      ...
      if (TopLLCMissInstrs[i]->getOperand(1).getReg()==LoopInductionInstr->getOperand(1).getReg()){
        ...
        llvm::outs()<<"[InjectPrefetchOuterLoopPass] TopLLCMiss instr must contain loop induction var\n";
        exit(1);
      }
...
/*
  MCInst Lea64rInstr2;
  if (TopLLCMissInstr->getOperand(1).getReg()==LoopInductionInstr->getOperand(1).getReg()){
    BC.MIB->createLEA64r(Lea64rInstr2, 
                         TopLLCMissInstr->getOperand(1).getReg(), 
                         1,  
                         BC.MIB->getNoRegister(), 
                         prefetchDist+64, 
  ...
  PrefetchBBs.back()->addInstruction(Lea64rInstr2);
...
BinaryBasicBlock* InjectPrefetchOuterLoopPass::createPopRegBB(BinaryFunction& BF,
                                                              BinaryBasicBlock* TargetBB,
                                                              BinaryBasicBlock* BoundsCheckBB,
  ...
  // mov 0x200(%r9,%rdx,8),%rax 
  // prefetcht0 (%rax) 
  MCSymbol *PopRegBBLabel = BC.Ctx->createNamedTempSymbol("PopRegBB");
  std::vector<std::unique_ptr<BinaryBasicBlock>> PopRegBBs;
  PopRegBBs.emplace_back(BF.createBasicBlock(BinaryBasicBlock::INVALID_OFFSET, PopRegBBLabel));
  PopRegBBs.back()->addSuccessor(TargetBB, 0,0);
  PopRegBBs.back()->addPredecessor(BoundsCheckBB);
  PopRegBBs.back()->addPredecessor(PrefetchBB);


  MCInst PopRegInstr;
  BC.MIB->createPopRegister(PopRegInstr, freeReg, 8);
  PopRegBBs.back()->addInstruction(PopRegInstr);

  // create unconditional branch at the end of 
  // prefetchBB
  PopRegBBs.back()->addBranchInstruction(TargetBB);  
  BF.insertBasicBlocks(BoundsCheckBB, std::move(PopRegBBs));

  // add PopRegBB to be the successor of the BoundsCheckBB
  BinaryBasicBlock* PopRegBB = BF.getBasicBlockForLabel(PopRegBBLabel);    
...
std::vector<std::string> InjectPrefetchOuterLoopPass::splitLine(std::string str){
...
std::unordered_map<std::string, std::vector<uint64_t>> InjectPrefetchOuterLoopPass::getTopLLCMissLocationFromFile(){
...
std::string InjectPrefetchOuterLoopPass::removeSuffix(std::string FuncName){
...
void InjectPrefetchOuterLoopPass::runOnFunctions(BinaryContext &BC) {
